...
class TestRunHandlerRegistry:
    ...
    def test_register(self, registry):
        registry.register(VALIDATE, RESULT_WRAPPED, ["test"])
        assert registry.registry[VALIDATE][RESULT_WRAPPED] == ["test"]
        registry.register(VALIDATE, RESULT_WRAPPED, "test2")
        assert registry.registry[VALIDATE][RESULT_WRAPPED] == [
            "test",
            "test2",
        ]
    ...
    def test_get_object(self, registry):
        ...
        registry.register(VALIDATE, RESULT_WRAPPED, ["test"])
        print(registry.registry)
        x = registry.get_object(VALIDATE, RESULT_WRAPPED)
        assert x == ["test"]
