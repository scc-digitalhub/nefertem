...
*Nefertem* allows passing credentials to backend storages through the ``Client``.
...
.. code-block:: python
   ...
   metadata_store = nt.StoreConfig(name="name", uri="uri", config=API_CREDENTIALS)
   client = nt.Client(project_name=project_name,
...
*Nefertem* offers seven plugin out-of-the-box for the *Artifact stores*.
...
.. code-block:: python
   ...
   store = nt.StoreConfig(name="name", type=STORE_TYPE, uri=URI, config=CREDENTIALS)
   client = nt.Client(store=store)
